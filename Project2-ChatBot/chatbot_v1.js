...
async function identify_user(agent)
{
   ...
   acct_num = agent.parameters.acct_num;
...
function user_registration(agent){
 ...
 MongoClient.connect(url,{ useUnifiedTopology: true }, function(err, db) {
  if (err) throw err;
    ...
    dbo.collection("user_table").insertOne({"username":user_name,"acct_num":acct_num}, function(err, res) {
...
function report_issue(agent)
{
  ...
  if(intent_val>4){
    agent.add("Invalid!Please re-enter the choice:");
  }
  else{
...
intentMap.set('service_intent - custom-2', user_registration);
